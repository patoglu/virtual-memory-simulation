...
typedef struct page_table_entry_t1
{
    ...
    int referenced;
}page_table_entry_NRU;

typedef struct page_table_entry_t2
{
    unsigned int page_frame_number; //which physical address it's maps.
    int present_absent; // is the data in memory or in disk.
    int dirty_bit; //is the page modified or not
}page_table_entry_FIFO;

typedef struct page_table_entry_t3
{
    unsigned int page_frame_number; //which physical address it's maps.
    int present_absent; // is the data in memory or in disk.
    int dirty_bit; //is the page modified or not
...
typedef struct page_table_entry_t4
{
    unsigned int page_frame_number; //which physical address it's maps.
    int present_absent; // is the data in memory or in disk.
    int dirty_bit; //is the page modified or not
}page_table_entry_LRU;

typedef struct page_table_entry_t5
{
    unsigned int page_frame_number; //which physical address it's maps.
    int present_absent; // is the data in memory or in disk.
    int dirty_bit; //is the page modified or not
}page_table_entry_WSClock;


typedef struct page_table_t1
{
    page_table_entry_NRU *table;
    int num_virtual;
...
typedef struct page_table_t2
{
    page_table_entry_FIFO *table;
    int capacity;
}page_table_FIFO;

typedef struct page_table_t3
{
    page_table_entry_SC *table;
    int capacity;
}page_table_SC;

typedef struct page_table_t4
{
    page_table_entry_LRU *table;
    int capacity;
}page_table_LRU;

typedef struct page_table_t5
{
    page_table_entry_WSClock *table;
    int capacity;
}page_table_WSClock;
...
extern page_table_NRU *v_page_table_NRU;
