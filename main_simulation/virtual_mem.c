...
int *index_sort(int b[], int len)
{
   ...
   int *return_index = malloc(sizeof(int) * (v_page_table_NRU->num_virtual * v_page_table_NRU-> frame_size / 4));
   for(i = 0 ; i < (v_page_table_NRU->num_virtual * v_page_table_NRU-> frame_size / 4) ;++i)
   {
       return_index[i] = b[i];
   }
...
void merging(int low, int mid, int high)
{
    ...
    int b[(v_page_table_NRU -> num_virtual) * (v_page_table_NRU -> frame_size) / 4  - 1];
    int l1,l2,i;
                ...
                unsigned int page_table_print_int)
{
    ...
    if((strcmp(page_replacement, "LRU") == 0) && (strcmp(alloc_policy, "local") == 0))
    {
        ...
        strcpy(alloc_policy, "global");
    }

}


void print_page_table_NRU(page_table_NRU *virtual_page_table)
{
...
fill_virtual_memory(unsigned int frame_size, unsigned int num_physical, unsigned int num_virtual, char *disk_name)
{
    ...
    if(strcmp(v_page_table_NRU-> alloc_policy, "global") == 0)
    {
        ...
        for(i = 0 ; i < num_virtual ; ++i)
        {
            ...
            {
                v_page_table_NRU -> table[i].page_frame_number = i; // map the physical address
                v_page_table_NRU -> table[i].present_absent = 1; // maps to real physical address so yes.
                v_page_table_NRU -> table[i].dirty_bit = 0; //not modified so no.
                v_page_table_NRU -> table[i].referenced = 0;
                enQueue(i);
                set_i_nth_page(i, v_page_table_NRU->num_virtual,"global", -1);
            ...
            {
                v_page_table_NRU -> table[i].page_frame_number = NOTPRESENTINMEMORY; //set the page frame number as not present in memory.
                v_page_table_NRU -> table[i].present_absent = 0; // The entry is in disk so set it to 0.
                v_page_table_NRU -> table[i].dirty_bit = 0; //Entry on disk and not even modified so set it to 0.
                v_page_table_NRU -> table[i].referenced = 0; //Entry on disk and not even referenced so set it to 0.
    ...
    else if(strcmp(v_page_table_NRU-> alloc_policy, "local") == 0)
    {
        ...
        int page_count = 0;
        //printf("??\n" );
        if(v_page_table_NRU -> num_physical < 4)
        {
        ...
        for(i = 0 ; i < v_page_table_NRU -> num_virtual / 4 ; ++i)
        {
            ...
            if(page_count < num_physical / 4)
            {
                ...
                v_page_table_NRU -> table[i].page_frame_number = k++; // map the physical address
                v_page_table_NRU -> table[i].present_absent = 1; // maps to real physical address so yes.
                v_page_table_NRU -> table[i].dirty_bit = 0; //not modified so no.
                v_page_table_NRU -> table[i].referenced = 0;
                enQueue_1(i);
                set_i_nth_page_first_quarter(i, v_page_table_NRU->num_virtual);
        ...
        for(i =  v_page_table_NRU -> num_virtual / 4 ; i <  v_page_table_NRU -> num_virtual / 2 ; ++i )
        {
            //printf("Disk offset is %d\n",disk_offset );
            if(page_count < num_physical / 4)
            {
                ...
                set_i_nth_page_second_quarter(i - v_page_table_NRU -> num_virtual / 4, v_page_table_NRU->num_virtual);
                for(j = 0 ; j < frame_size ; ++j)
                {
                    ...
                    random_number = rand();
                    //printf("Generated random number %d\n",rand());
                    physical_mem[j + (i - v_page_table_NRU -> num_virtual / 4 + num_physical / 4) * frame_size] = random_number; // initialize physical memory.
                    fprintf(DISK, "%d\n",physical_mem[j + (i - v_page_table_NRU -> num_virtual / 4  + num_physical / 4) * frame_size]); //initialize same contents to disk too.
                    disk_offset += _JUMP;
                    fseek(DISK, disk_offset, SEEK_SET); //Seek forward to create some kind of boundary between integers in file.
                    ...
                    random_number = rand();
                    printf("Generated random number %d\n",rand());
                    physical_mem[j + (i - v_page_table_NRU -> num_virtual / 2 + num_physical / 2) * frame_size] = random_number; // initialize physical memory.
                    fprintf(DISK, "%d\n",physical_mem[j + (i - v_page_table_NRU -> num_virtual / 2 + num_physical / 2) * frame_size]); //initialize same contents to disk too.
                    disk_offset += _JUMP;
        ...
        for(i =  v_page_table_NRU -> num_virtual / 2 ; i <  v_page_table_NRU -> num_virtual * 3 / 4 ; ++i )
        {
            //printf("Disk offset is %d\n",disk_offset );
            if(page_count < num_physical / 4)
            {
                ...
                v_page_table_NRU -> table[i].referenced = 0;
                enQueue_3(i);
                set_i_nth_page_third_quarter(i - v_page_table_NRU -> num_virtual / 2, v_page_table_NRU->num_virtual);
        ...
        for(i =  v_page_table_NRU -> num_virtual * 3 / 4 ; i <  v_page_table_NRU -> num_virtual ; ++i )
        {
            //printf("Disk offset is %d\n",disk_offset );
            if(page_count < num_physical / 4)
            {
                ...
                v_page_table_NRU -> table[i].referenced = 0;
                enQueue_4(i);
                set_i_nth_page_fourth_quarter(i - v_page_table_NRU -> num_virtual * 3 / 4, v_page_table_NRU->num_virtual);
                for(j = 0 ; j < frame_size ; ++j)
                {
                    //fprintf(stdout, "%d\n",random_number );
                    ...
                    random_number = rand();
                    //printf("Generated random number %d\n",rand());
                    physical_mem[j + (i - v_page_table_NRU -> num_virtual * 3 / 4 + num_physical * 3 / 4) * frame_size] = random_number; // initialize physical memory.
                    fprintf(DISK, "%d\n",physical_mem[j + (i - v_page_table_NRU -> num_virtual * 3 / 4 + num_physical * 3 / 4) * frame_size]); //initialize same contents to disk too.
                    disk_offset += _JUMP;
                    fseek(DISK, disk_offset, SEEK_SET); //Seek forward to create some kind of boundary between integers in file.
                }
            }
...
void
create_page_table_NRU(unsigned int frame_size,
                     unsigned int num_physical,
                     unsigned int num_virtual,
                     char page_replacement[10],
                     char alloc_policy[10],
                     ...
                     char disk_name[30])
{
    ...
    v_page_table_NRU =  calloc(1,sizeof(page_table_NRU));
    if(v_page_table_NRU == NULL)
    {
        fprintf(stderr, "%s\n", "malloc fail");
    }
    v_page_table_NRU -> table = calloc(num_virtual, sizeof(page_table_entry_NRU) );
    if(v_page_table_NRU == NULL)
    {
        fprintf(stderr, "%s\n", "malloc fail");
    }
    v_page_table_NRU -> num_virtual = num_virtual;
    v_page_table_NRU -> frame_size = frame_size;
    v_page_table_NRU -> num_physical = num_physical;
    v_page_table_NRU -> page_table_print_int = page_table_print_int;
    strcpy(v_page_table_NRU -> page_replacement, page_replacement);
    strcpy(v_page_table_NRU -> alloc_policy, alloc_policy);
    strcpy(v_page_table_NRU -> disk_name, disk_name);
...
void
free_page_table_NRU(void)
{
    free(v_page_table_NRU -> table);
    free(v_page_table_NRU);
}
...
int get(unsigned int index, char * tName)
{
    ...
    int offset;
    int i,j;
    int num_physical = v_page_table_NRU -> num_physical;
    int num_virtual = v_page_table_NRU -> num_virtual;
    int frame_size = v_page_table_NRU -> frame_size;
    int page_table_print_int = v_page_table_NRU -> page_table_print_int;
    strcpy(page_replacement, v_page_table_NRU -> page_replacement);
    strcpy(alloc_policy, v_page_table_NRU -> alloc_policy);
    strcpy(disk_name, v_page_table_NRU -> disk_name);
    ...
    if(strcmp(page_replacement, "NRU") == 0)
    {
        ...
        {
            index += v_page_table_NRU->num_virtual * v_page_table_NRU-> frame_size / 2;

        }

        else if(strcmp(tName, "Index") == 0)
        {
            index += v_page_table_NRU->num_virtual * v_page_table_NRU-> frame_size / 4 * 3;
        ...
        if(strcmp(tName, "Bubble") == 0)
        {
                    ...
                    if(v_page_table_NRU -> table[page_no].present_absent == 1)
                    {
                        ...
                        fclose(DISK);
                        return_value = physical_mem[v_page_table_NRU -> table[page_no].page_frame_number * frame_size + offset];
                        pthread_mutex_unlock(&lock);
                        return return_value;
                    }
                    //if the specified page is not present in virtual page well, there we go...
                    else if(v_page_table_NRU -> table[page_no].present_absent == 0)
                    {
                        ...
                        //IN NRU Page Replacement Algorithm we divide pages as 4 categories:
                            ...
                            if(strcmp(v_page_table_NRU -> alloc_policy, "global") == 0)
                            {
                                ...
                                if((found_first_class == 0))
                                {
                                    for(i = 0 ; (i < num_virtual)  ; ++i)
                                    {
                                        if(v_page_table_NRU->table[i].present_absent == 1
                                           && v_page_table_NRU->table[i].referenced == 0
                                           && v_page_table_NRU->table[i].dirty_bit == 1)
                                        {
                                            //printf("Little\n");
                                               //found class 1;
                                ...
                                if((found_second_class == 0) && (found_first_class == 0))
                                {
                                    ...
                                    {
                                        if(v_page_table_NRU->table[i].present_absent == 1
                                           && v_page_table_NRU->table[i].referenced == 1
                                           && v_page_table_NRU->table[i].dirty_bit == 0)
                                        {
                                            //printf("Birds\n");
                                               //found class 1;
                            ...
                            old_address = v_page_table_NRU -> table[i].page_frame_number; //save page no.

                            v_page_table_NRU -> table[i].present_absent = 0; // This victim page will be in disk afterwards.
                            if(v_page_table_NRU -> table[i].dirty_bit == 1) //If the page is dirty we need to write back into disk.
                            {
                                disk_page_writes_bubble_get++;
                                v_page_table_NRU -> table[i].dirty_bit = 0; //Victim page is not dirty anymore.
                                disk_offset = i * frame_size * _JUMP; //move offset to victim page.
                                fseek(DISK, disk_offset, SEEK_SET);
                                for(j = 0 ; j < frame_size ; ++j)
                                {
                                    //store the contents of that page into disk.
                                    fprintf(DISK, "%d\n",physical_mem[(v_page_table_NRU -> table[i].page_frame_number * frame_size) + j]);
                                    disk_offset += _JUMP;
                                    fseek(DISK, disk_offset, SEEK_SET);
                                }
                            }

                            v_page_table_NRU -> table[page_no].present_absent = 1; //new page will be in physical memory
                            v_page_table_NRU ->table[page_no].page_frame_number = old_address;//restore page no.
                            disk_offset = page_no * frame_size * _JUMP;
                            v_page_table_NRU -> table[i].page_frame_number = NOTPRESENTINMEMORY;
                            disk_page_reads_bubble_get++;
                            ...
                            {
                                fscanf(DISK, "%d\n", &number);
                                physical_mem[v_page_table_NRU -> table[page_no]. page_frame_number * frame_size + i] = number;
                                disk_offset += _JUMP;
                                fseek(DISK, disk_offset, SEEK_SET);
                            }
                            fclose(DISK);
        ...
        if(strcmp(tName, "Bubble") == 0)
        {
                    ...
                    else if(v_page_table_NRU -> table[page_no].present_absent == 0)
                    {
                        ...
                        //IN NRU Page Replacement Algorithm we divide pages as 4 categories:
                            ...
                            if(strcmp(v_page_table_NRU -> alloc_policy, "local") == 0)
                            {
                                enQueue_1(page_no);
                            }
                            v_page_table_NRU -> table[page_no].present_absent = 1; //new page will be in physical memory
    ...
    else if(strcmp(page_replacement, "LRU") == 0)
    {
        ...
        if(strcmp(tName, "Bubble") == 0)
        {
                    ...
                    if(strcmp(v_page_table_NRU -> alloc_policy, "global") == 0)
                    {
                        set_i_nth_page(page_no, v_page_table_NRU -> num_virtual, "global", -1); // set referenced for LRU
                    }
                    else if(strcmp(v_page_table_NRU -> alloc_policy, "local") == 0)
                    {
                        int temp_1;
                        if(page_no >= 0 && page_no <  v_page_table_NRU -> num_virtual / 4)
                        {

                            set_i_nth_page_first_quarter(page_no, v_page_table_NRU->num_virtual);
                        }
                        else if(page_no >= v_page_table_NRU -> num_virtual / 4 && page_no < v_page_table_NRU -> num_virtual / 2 )
                        {
                            temp_1 = page_no -v_page_table_NRU -> num_virtual / 4;
                            set_i_nth_page_second_quarter(temp_1, v_page_table_NRU->num_virtual);
                        }
                        else if(page_no >= v_page_table_NRU -> num_virtual / 2 && page_no < v_page_table_NRU -> num_virtual * 3 / 4 )
                        {
                            temp_1 = page_no - v_page_table_NRU -> num_virtual / 2;
                            set_i_nth_page_third_quarter(temp_1, v_page_table_NRU->num_virtual);
                        }
                        else if(page_no >= v_page_table_NRU -> num_virtual * 3 / 4 && page_no < v_page_table_NRU -> num_virtual)
                        {
                            temp_1 = page_no -v_page_table_NRU -> num_virtual * 3 / 4;
                            set_i_nth_page_fourth_quarter(temp_1, v_page_table_NRU->num_virtual);
                        }
                    }
                    ...
                    else if(v_page_table_NRU -> table[page_no].present_absent == 0)
                    {
                        ...
                        //IN NRU Page Replacement Algorithm we divide pages as 4 categories:
                            ...
                            if(strcmp(v_page_table_NRU -> alloc_policy, "global") == 0)
                            {
                                ...
                                i = get_LRU_page(v_page_table_NRU -> num_virtual,"global",-1);
                            ...
                            else if(strcmp(v_page_table_NRU -> alloc_policy, "local") == 0)
                            {
                                ...
                                if(page_no >= 0 && page_no <  v_page_table_NRU -> num_virtual / 4)
                                {
                                    ...
                                    i = get_LRU_page_first(v_page_table_NRU -> num_virtual);
                                }
                                else if(page_no >= v_page_table_NRU -> num_virtual / 4 && page_no < v_page_table_NRU -> num_virtual / 2 )
                                {
                                    i = get_LRU_page_second(v_page_table_NRU -> num_virtual);
                                }
                                else if(page_no >= v_page_table_NRU -> num_virtual / 2 && page_no < v_page_table_NRU -> num_virtual * 3 / 4 )
                                {
                                    i = get_LRU_page_third(v_page_table_NRU -> num_virtual);
                                }
                                else if(page_no >= v_page_table_NRU -> num_virtual * 3 / 4 && page_no < v_page_table_NRU -> num_virtual)
                                {
                                    i = get_LRU_page_fourth(v_page_table_NRU -> num_virtual);
                                }
    ...
    if(strcmp(page_replacement, "NRU") == 0)
    {
        ...
        if(strcmp(tName, "Bubble") == 0)
        {
            ...
            if(v_page_table_NRU -> table[page_no].present_absent == 1)
            {
                ...
                //printf("%d. index in physical memory will be set \n",v_page_table_NRU -> table[page_no].page_frame_number * frame_size + offset);
                //don't forget to set referenced and dirty bit
                v_page_table_NRU -> table[page_no].referenced = 1;
                v_page_table_NRU -> table[page_no].dirty_bit = 1;
                fclose(DISK);
                physical_mem[v_page_table_NRU -> table[page_no].page_frame_number * frame_size + offset] = value;
                pthread_mutex_unlock(&lock);
        ...
        if(strcmp(tName, "Bubble") == 0)
        {
            ...
            else if(v_page_table_NRU -> table[page_no].present_absent == 0)
            {
                ...
                //IN NRU Page Replacement Algorithm we divide pages as 4 categories:
                    ...
                    else if(strcmp(v_page_table_NRU -> alloc_policy, "global") == 0)
                    {
                        enQueue(page_no);
                    }
...
void *thread_1()
{
    ...
    for(i = 0 ; i < (v_page_table_NRU->num_virtual * v_page_table_NRU-> frame_size / 4) - 1 ; ++i)
    {
        for(j = 0 ; j < (v_page_table_NRU->num_virtual * v_page_table_NRU-> frame_size / 4) - i - 1 ; j ++)
        {
...
{
    int size;
    size = (v_page_table_NRU->num_virtual * v_page_table_NRU-> frame_size / 4);
...
void *thread_4()
{
    int b[(v_page_table_NRU->num_virtual * v_page_table_NRU-> frame_size / 4)];
    indices = index_sort(b, (v_page_table_NRU->num_virtual * v_page_table_NRU-> frame_size / 4));
...
int get_LRU_page(int page_no, char *alloc_policy, int which_quarter)
{
    ...
    if(strcmp(alloc_policy, "global") == 0)
    {
        for(i = 0 ; i < v_page_table_NRU -> num_virtual ; ++i)
        {
            if(v_page_table_NRU -> table[i]. present_absent == 1)
            {
                present_pages[k++] = i;
        ...
        for(i = 0 ; i < v_page_table_NRU -> num_physical ; ++i)
        {
            for(j = 0 ; j < v_page_table_NRU -> num_virtual ; ++j)
            {
    ...
    else if(strcmp(alloc_policy, "local") == 0)
    {
        ...
        if(which_quarter == 1)
        {
            ...
            for(i = 0 ; i < v_page_table_NRU -> num_physical / 4 ; ++i)
            {
            ...
            for(i = 0 ; i < v_page_table_NRU -> num_physical / 4 ; ++i)
            {
                for(j = 0 ; j < v_page_table_NRU -> num_virtual / 4 ; ++j)
                {
                    if(tanenbaum_LRU_matrix[present_pages[i]][j] == 1 )
                    {
                        fewest_ones++;
        ...
        {
            k = v_page_table_NRU -> num_virtual / 4;
            for(i = v_page_table_NRU -> num_virtual / 4 ; i < v_page_table_NRU -> num_virtual / 2 ; ++i)
            {
                if(v_page_table_NRU -> table[i]. present_absent == 1)
                {
            ...
            for(i = v_page_table_NRU -> num_physical / 4; i < v_page_table_NRU -> num_physical / 2 ; ++i)
            {
                for(j =  v_page_table_NRU -> num_virtual / 4 ; j < v_page_table_NRU -> num_virtual / 2 ; ++j)
                {
                    if(tanenbaum_LRU_matrix[present_pages[i]][j] == 1 )
                    {
                        fewest_ones++;
        ...
        {
            k = v_page_table_NRU -> num_virtual  * 3 / 4;
            for(i = v_page_table_NRU -> num_virtual / 2 ; i < v_page_table_NRU -> num_virtual * 3 / 4 ; ++i)
            {
                if(v_page_table_NRU -> table[i]. present_absent == 1)
                {
            ...
            for(i = v_page_table_NRU -> num_physical / 2; i < v_page_table_NRU -> num_physical * 3 / 4 ; ++i)
            {
                for(j =  v_page_table_NRU -> num_virtual / 2 ; j < v_page_table_NRU -> num_virtual  * 3 / 4  ; ++j)
                {
                    if(tanenbaum_LRU_matrix[present_pages[i]][j] == 1 )
                    {
                        fewest_ones++;
        ...
        else if(which_quarter == 4)
        {
            ...
            for(i = v_page_table_NRU -> num_physical * 3 / 4 ; i < v_page_table_NRU -> num_physical  ; ++i)
            {
            ...
            for(i =  v_page_table_NRU -> num_physical * 3 / 4 ; i < v_page_table_NRU -> num_physical ; ++i)
            {
                for(j = v_page_table_NRU -> num_virtual * 3 / 4 ; j < v_page_table_NRU -> num_virtual ; ++j)
                {
                    if(tanenbaum_LRU_matrix[present_pages[i]][j] == 1 )
                    {
                        fewest_ones++;
